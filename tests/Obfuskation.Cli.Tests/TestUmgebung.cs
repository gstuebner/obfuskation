...
///
/// <see cref="Obfuskation.Core.PathHelper.ConfigDirectory"/> loest ueber
/// <c>XDG_CONFIG_HOME</c> auf, <see cref="Obfuskation.Core.PathHelper.DefaultMappingStorePath"/>
/// ueber <c>XDG_DATA_HOME</c>. Ohne diese Umleitung wuerden die Tests zur
/// Namensauflösung in <see cref="CommandContext.LoadProfile"/> (die
/// absichtlich den zentralen Profilordner benutzen, um genau diesen
/// Mechanismus zu pruefen) in <c>~/.config/obfuskation</c> schreiben und
/// lesen.
///
...
internal static class TestUmgebung
{
    ...
    {
        _basisVerzeichnis = Path.Combine(Path.GetTempPath(), "obfuskation-cli-testkonfiguration",
