...
**Colour shows what was detected.** Pasted text appears on the left as a review pane: green for what is detected and replaced, orange for what a cleared checkbox deliberately keeps in the clear; the right-hand side carries the same colours on the replacement values. **No colour** means no rule matched it — that text would go out unchanged. Detection covers IBAN, e-mail, BIC and phone numbers; names, addresses, dates of birth, tax IDs and customer numbers it does not see, and no regex catches those reliably. *Edit* switches to an ordinary text box, *Done* switches back.

**Marking what was missed — without a regex.** If something confidential is still sitting there uncoloured, select it and right-click **"»…« immer ersetzen…"** — the entry names the selected value. Or press **Ctrl+M**. The dialog asks three things in plain language: *what* (prefilled from the selection), *how far* (the literal word, or — if it contains digits — "everything of this shape", e.g. `FW123456` becomes "„FW“ + 6 digits" instead of `\bFW\d{6}\b`, with a live match count against the current text), and *where* (this project's profile, or the extension file for every project — the dialog names the target path and warns before it overwrites hand-written comments there with a backup). The file view carries the same dialog as *Always Replace…* next to *Auto-detect Fields…*.

One rule often covers several occurrences at once; for several genuinely different terms in a row, *Apply and continue* keeps the dialog open. A rule you created yourself can be deleted again from the match list via *Remove rule* — pseudonyms already issued stay in the mapping store, so the way back to the real values is never lost. *Edit patterns manually…* still reaches the full rule editor with its test field.
